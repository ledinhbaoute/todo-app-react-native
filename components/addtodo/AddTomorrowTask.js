...
import { List, Appbar, Text, Checkbox, Button } from 'react-native-paper';
import { useSelector, useDispatch } from 'react-redux';
import { AsyncStorage, View, TextInput, TouchableOpacity } from 'react-native';
import { nanoid } from '@reduxjs/toolkit'
import { addTask, updateTasks } from '../../redux/TasksSlice';
...
const AddTomorrowTask = ({ navigation }) => {
    ...
    const _handleAddTask = () => {
        if (taskName && time) {
            const newListTasks = tasks.concat({ id: nanoid(), name: taskName, time: time.toISOString(), note: note, isDone: false })
            ...
            <View className="container px-3">
                ...
                <View className="h-full max-h-80 w-full flex justify-content mt-4">
                    ...
                    <View className="mt-4 flex-row justify-center item-center">
                        <TouchableOpacity>
                            <PlusCircleIcon color={colors.orange} size="70" onPress={_handleAddTask}></PlusCircleIcon>
                        </TouchableOpacity>
                    </View>
                </View>
            </View>
